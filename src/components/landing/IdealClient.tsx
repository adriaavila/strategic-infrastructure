    ...
    <section id="cliente-ideal" className="relative py-24 md:py-32 scroll-mt-20 bg-muted/30">
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-8">
          <motion.div className="bg-card border border-brand-secondary/20 rounded-2xl p-8" initial={{ opacity: 0, x: -40 }} animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -40 }} transition={{ delay: 0.2, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <ul className="space-y-4">
              {idealClientPoints.map((point, index) => (
                <motion.li key={index} className="flex items-start gap-3" initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -20 }} transition={{ delay: 0.3 + index * 0.1, duration: 0.4 }}>
          ...
          <motion.div className="bg-card border border-foreground/10 rounded-2xl p-8" initial={{ opacity: 0, x: 40 }} animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: 40 }} transition={{ delay: 0.2, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
            <div className="flex items-center gap-3 mb-6">
